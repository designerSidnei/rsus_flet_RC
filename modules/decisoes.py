...
colunas_decisao = ['DECISÃO', 'ALEGAÇÃO1', 'ALEGAÇÃO2', 'ALEGAÇÃO3', 'MOTIVO1', 'MOTIVO2', 'MOTIVO3', 'VLR DECOTE',
                   'VLR REMANESCENTE']
...
async def mainn(path_plan, path_dados):
    ...
    if atendimento is None or competencia is None or tipo_procedimento is None:
        not_coluna = "do atendimento" if atendimento is None else "da competência" if competencia is None else \
            "do tipo de procedimento (Principal/Secundário)"
    ...
    for key in data_base.keys():
        for index, row in df.iterrows():
            ...
            if str(row[atendimento]) == str(data_base[key]['atendimento']) and row[tipo_procedimento] in lista_valor_tipo_procedimento:
                ...
                if row_competencia == comparar_comp:  # type: ignore
                    ...
                    df.at[index, 'DECISÃO'] = data_base[key]['opinion']  # type: ignore
                    ...
                    df.at[index, 'VLR DECOTE'] = data_base[key]['decote']  # type: ignore
                    df.at[index, 'VLR REMANESCENTE'] = data_base[key]['resto']  # type: ignore
